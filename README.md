...
```mermaid
graph TB
    A[Azure Subscription] --> B[Service Principal]
    B --> C[RBAC Roles]
    C --> D[Reader]
    C --> E[Billing Reader]
    C --> F[Security Reader]
    C --> G[Custom Firefly Role]

    A --> H[Monitoring Infrastructure]
    H --> I[Storage Account]
    H --> J[Event Grid Topic]
    H --> K[Diagnostic Settings]

    I --> L[Firefly Webhook]
    J --> L
    K --> I

    L --> M[Firefly Platform]
```

### **Multi-Subscription Architecture**
```mermaid
graph TB
    A[Subscription 1] --> D[Shared Service Principal]
    B[Subscription 2] --> D
    C[Subscription N] --> D

    D --> E[RBAC Roles per Subscription]

    A --> F[Monitoring Infrastructure 1]
    B --> G[Monitoring Infrastructure 2]
    C --> H[Monitoring Infrastructure N]

    F --> I[Firefly Platform]
    G --> I
    H --> I
